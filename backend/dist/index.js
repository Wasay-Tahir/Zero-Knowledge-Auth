...
app.post('/auth/verify', async (req, res) => {
    ...
    if (!expectedNonce)
        return res.status(400).json({ error: 'nonce missing/expired' });
    try {
        const ok = await verifyZkProof(proof, publicSignals);
        if (!ok)
...
app.post('/auth/login', async (req, res) => {
    ...
    if (!salt)
        return res.status(400).json({ error: 'salt required (not stored for this user)' });
    try {
        ...
        return res.status(500).json({ error: 'proving/verification error', details: e.message });
    }
});
app.listen(PORT, () => {
